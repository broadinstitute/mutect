...
//                );
    ...
    private String generateCallStats(CandidateMutation candidate) {
        ...
        String[] msg = new String[] {
                        ...
                        fmt.format(candidate.getLocation().getStart()),
                        ""+candidate.getRefAllele(),
                        ""+candidate.getAltAllele(),
                        candidate.getTumorSampleName(),
                        candidate.getNormalSampleName(),
                        fmt.format(candidate.getScore()),
                        siteInfo,
                        (candidate.isCovered()?"COVERED":"UNCOVERED"),
                        fmt.format(candidate.getPower()),
                        fmt.format(candidate.getTumorPower()),
                        fmt.format(candidate.getNormalPower()),
                        fmt.format(candidate.getTotalPairs()),
                        fmt.format(candidate.getImproperPairs()),
                        fmt.format(candidate.getMapQ0Reads()),
                        fmt.format(candidate.getInitialTumorLod()),
                        fmt.format(candidate.getTumorLodFStar()),
                        fmt.format(candidate.getTumorLodFStarForward()),
                        fmt.format(candidate.getTumorLodFStarReverse()),
                        fmt.format(candidate.getTumorF()),
                        fmt.format(FRACTION_CONTAMINATION),
                        fmt.format(candidate.getContaminantLod()),
                        fmt.format(candidate.getMinimumTumorAlleleFraction()),
                        fmt.format(candidate.getTumorQ20Count()),
                        fmt.format(candidate.getInitialTumorRefCounts()),
                        fmt.format(candidate.getInitialTumorAltCounts()),
                        fmt.format(candidate.getInitialTumorRefQualitySum()),
                        fmt.format(candidate.getInitialTumorAltQualitySum()),
                        fmt.format(candidate.getTumorInsertionCount()),
                        fmt.format(candidate.getTumorDeletionCount()),
                        fmt.format(candidate.getInitialNormalBestGenotype()),
                        fmt.format(candidate.getInitialNormalLod()),
                        fmt.format(candidate.getNormalLodFStar()),
                        fmt.format(candidate.getNormalF()),
                        fmt.format(candidate.getNormalQ20Count()),
                        fmt.format(candidate.getInitialNormalRefCounts()),
                        fmt.format(candidate.getInitialNormalAltCounts()),                        
                        fmt.format(candidate.getInitialNormalRefQualitySum()),
                        fmt.format(candidate.getInitialNormalAltQualitySum()),
                        fmt.format(candidate.isPositiveDirectionAtRisk()?1:0),
                        fmt.format(candidate.isNegativeDirectionAtRisk()?1:0),
                        fmt.format(candidate.isPositiveDirectionPowered()?1:0),
                        fmt.format(candidate.isNegativeDirectionPowered()?1:0),
                        fmt.format(candidate.getPerfectStrandBias().getP()),
                        fmt.format(candidate.getStrandBias().dataToString()),
                        fmt.format(candidate.getStrandBias().getP()),
                        fmt.format(classicSkewScore==null?"n/a":classicSkewScore),
                        fmt.format(classicSkewOffset==null?"n/a":classicSkewOffset),
                        fmt.format(fisherSkewScore==null?"n/a":fisherSkewScore),
                        fmt.format(fisherSkewOffset==null?"n/a":fisherSkewOffset),
                        fmt.format(qrst==null?"n/a":candidate.getTumorQualityRankSumTest().getMedianShift()),
                        fmt.format(qrst==null?"n/a":candidate.getTumorQualityRankSumTest().getP()),
                        fmt.format(prst==null?"n/a":candidate.getTumorReadPositionRankSumTest().getMedianShift()),
                        fmt.format(prst==null?"n/a":candidate.getTumorReadPositionRankSumTest().getP()),
                        fmt.format(candidate.getTumorForwardOffsetsInReadMedian()==null?"n/a":candidate.getTumorForwardOffsetsInReadMedian()),
                        fmt.format(candidate.getTumorForwardOffsetsInReadMad()==null?"n/a":candidate.getTumorForwardOffsetsInReadMad()),
                        fmt.format(candidate.getTumorReverseOffsetsInReadMedian()==null?"n/a":candidate.getTumorReverseOffsetsInReadMedian()),
                        fmt.format(candidate.getTumorReverseOffsetsInReadMad()==null?"n/a":candidate.getTumorReverseOffsetsInReadMad()),
                        ...
                        fmt.format(candidate.getCountOfNormalsObservedIn()),
